...
while True:
    q_values = pickle.load(open("connect4/trainingdata.p", "rb"))
    ...
    pickle.dump(game.q_values, open("connect4/trainingdata.p", "wb"))
    json_q_values = {}
    for gamestate in sorted(q_values):
        help_vals = {}
        for item in q_values.get(gamestate):
            help_vals[item]=q_values[gamestate][item]
        json_q_values[""+gamestate] = help_vals

    json.dump(json_q_values, open("connect4/trainingdata.json", "w"), ensure_ascii=False, sort_keys=True, indent=4)
